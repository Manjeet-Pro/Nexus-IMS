...
exports.getParentDashboard = async (req, res) => {
    try {
        ...
        const enhancedChildren = await Promise.all(parent.children.map(async (child) => {
            ...
            const totalClasses = await require('../../8_Attendance_Module/backend/Attendance').countDocuments({ student: child._id });
            const attendedClasses = await require('../../8_Attendance_Module/backend/Attendance').countDocuments({ student: child._id, status: 'Present' });
            const attendancePercentage = totalClasses > 0 ? Math.round((attendedClasses / totalClasses) * 100) : 0;

            // 2. Get Recent Performance (Average of last 5 marks)
            const recentMarks = await require('../../9_Examination_Marksheet_Module/backend/Mark').find({ student: child._id })
                .sort({ date: -1 })
                .limit(5);
...
exports.getChildDetails = async (req, res) => {
    try {
        ...
        const results = await require('../../9_Examination_Marksheet_Module/backend/Mark').find({ student: id })
        ...
        const courses = await require('../../6_Academic_Course_Management_Module/backend/Course').find({ studentsEnrolled: id })
...
exports.getChildFees = async (req, res) => {
    try {
        ...
        const fees = await require('../../10_Finance_Fee_Management_Module/backend/Fee').find({ student: childId }).sort({ dueDate: 1 });
        res.json(fees);
    } catch (error) {
...
exports.getParentNotices = async (req, res) => {
    try {
        ...
        const notices = await require('../../11_Communication_Notice_Board_Module/backend/Notice').find({
...
exports.updateChildAvatar = async (req, res) => {
    try {
        ...
        const user = await require('../../1_Authentication_Core_Security_Module/backend/User').findById(student.user);
        if (user) {
            user.avatar = avatar;
            await user.save();
