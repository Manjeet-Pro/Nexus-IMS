...
const { notifyMany } = require('./notificationHelper');
...
exports.chatFAQ = async (req, res) => {
    try {
        ...
        if (!userMessage) return res.status(400).json({ message: "Empty message" });

        const systemPrompt = `You are Nexus AI, a professional educational assistant for the NEXUS Institute Management System. 
        User: ${req.user.name} (Role: ${req.user.role})
        
        RULES:
        1. ONLY answer questions related to Computer Science, Programming, and Institute operations (Fees, Attendance, notices).
        2. If the user asks anything else, politely decline.
        
        User Query: "${userMessage}"`;

        // Using gemini-flash-latest which is confirmed available in the user's model list
        ...
        console.error("AI API ERROR LOG:", JSON.stringify(errorData));
        
        // Return a cleaner message to the user pointing to the specific error
        const userFriendlyMsg = error.response?.data?.error?.message 
            ? `AI Brain Error: ${error.response.data.error.message}`
            : "Encountered an error connecting to AI brain. Please try again or check the API key.";
            
        res.status(500).json({ message: userFriendlyMsg }); 
    }
};
