...
const Sidebar = ({ user, onClose }) => {
    ...
    const roleLinks = {
        ...
        faculty: [
            ...
            { to: '/faculty/courses', icon: FileText, label: 'My Courses' },
            { to: '/faculty/timetable', icon: Calendar, label: 'Timetable' },
            { to: '/faculty/directory', icon: Users, label: 'Student Directory' },
            { to: '/faculty/marks', icon: Award, label: 'Marks Entry' },
        ],
        student: [
            { to: '/student', icon: LayoutDashboard, label: 'Dashboard', end: true },
            { to: '/student/courses', icon: BookOpen, label: 'My Courses' },
            { to: '/student/attendance', icon: Clock, label: 'Attendance' },
            { to: '/student/fees', icon: IndianRupee, label: 'My Fees' },
            { to: '/student/exams', icon: Calendar, label: 'Exams' },
            ...
            <div className="h-20 flex items-center px-6 border-b border-gray-100 dark:border-gray-800">
                <div className="flex items-center gap-3 group cursor-pointer" onClick={() => navigate('/')}>
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-[10px] font-bold tracking-widest text-primary-600 dark:text-primary-400 uppercase">{user?.role}</span>
                    </div>
                </div>
            </div>
